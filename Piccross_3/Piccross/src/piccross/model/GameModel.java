...
public class GameModel {
	...
	/** 2d array to store user draw game */
	...
	/**
	 * Set the dimension of the drawing game board.
	 * @param d the dimension of board
	 */
	public void setDrawGameDimension(int d) {
		drawGrid = new boolean[d][d];
		for(int i = 0; i < d; i++) {
			for(int j = 0; j < d; j++) {
				drawGrid[i][j] = false;
			}
	...
	/**
	 * Get the dimension of the draw game board.
	 * @return the draw board dimension
	 */
	public int getDrawGameDimension() {
		return drawGrid.length;
	}
	
	public void setDrawCellMarked(int i, int j, boolean marked) {
		drawGrid[i][j] = marked;
	}
	public boolean getDrawCellMarked(int i, int j) {
		return drawGrid[i][j];
	}
	
	public GameConfiguration getDrawGameConfiguration() {
		String s = "";
		int d = getDrawGameDimension();
		for(int i = 0; i < d; i++) {
			for(int j = 0; j < d; j++) {
				s += (drawGrid[i][j])?"1":"0";
			}
		...
		return GameConfiguration.fromString(s);
	}
